...
onload = function () 
{
    ...
    function getData()
    {
        var sz = parseInt(prompt("Enter the number of people : "));
        let nodes = [];
        for(let i=1;i<=sz;i++)
        {
            nodes.push({id:i, label:"Person "+i})
        ...
        for(let i=1;i<=sz;i++)
        {
            for(let j=i+1;j<=sz;j++)
            {
               var ed= parseInt(prompt("Enter the amount (if any) that Person "+ i +" needs to pay to person "+ j + ". Or if the amount needs to be paid by Person "+ j+" to Person "+ i +", then enter the amount in negative. If noone owes nothing, then type 0."));
                    if(ed>0)
                        edges.push({from: i, to: j, label: String(ed)});
                    else if(ed<0)
                        edges.push({from: j, to: i, label: String(ed*(-1))});
             }
        }
        const data = 
         {
